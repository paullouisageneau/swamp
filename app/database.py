...
class Database:
	...
	def init(self):
		...
		c.execute("CREATE UNIQUE INDEX IF NOT EXISTS access_user_index ON access(user_id, directory_id)")
	...
	def setDirectoryAccess(self, path, username, level):
		...
		directory_id = r[0]
		c.execute("INSERT OR REPLACE INTO access (user_id, directory_id, level) VALUES (?, ?, ?)", (user_id, directory_id, level))
		self._conn.commit()
