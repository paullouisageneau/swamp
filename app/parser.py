...
class Parser:
    ...
    PatternSerie = re.compile('(?:\[(.*)\]\.?)?([^\[\]]+)(?:\.|\-)(?:S([0-9]{2}) ?E([0-9]{2})|([0-9]{1,2})X([0-9]{2}))(?:(?:\.|\-)(.*))?\.('+PatternVideoExt+')$', re.IGNORECASE)
    PatternMovie = re.compile('(?:\[(.*)\]\.?)?([^\[\]]+)(?:\.|\-)(\(?((?:19|20)[0-9]{2})\)?)(?:(?:\.|\-)(.*))?\.('+PatternVideoExt+')$', re.IGNORECASE)
    PatternMusic = re.compile('(?:\[(.*)\]\.?)?(?:([0-9]{1,3})(?:\-|\.))?(?:(.+[^0-9])\-)?(?:([0-9]{1,3})\-)?([^()]+)(?:\.\((.*)\))?\.('+PatternAudioExt+')$', re.IGNORECASE)
    ...
    def __init__(self, filename):
        ...
        if m:
            ...
            print("Serie: " + self.title + "\tseason=" + str(self.season) + "\tepisode=" + str(self.episode) + ("\ttag = " + self.tag if self.tag else ""))
            return
        ...
        if m:
            ...
            print("Movie: " + self.title + ("\ttag = " + self.tag if self.tag else ""))
            return
        ...
        if m:
            ...
            if self.track or self.artist:
                print("Music: " + self.title + ("\ttrack=" + str(self.track) if self.track > 0 else "") + ("\tartist=" + self.artist if self.artist else "") + ("\ttag = " + self.tag if self.tag else ""))
                return
